...
def video_content_scene_detector():
	directory_threshold = (os.getcwd() + str("/")+str("cropped_threshold")).replace("\\","/")
	for filename in os.listdir(directory_threshold):
	    if filename.endswith(".mp4"): 
	    	scenedetect_content_script = "scenedetect -i " + str((directory_threshold + str("/") + filename).replace("\\","/")) + " -o cropped_content detect-content -m 5s " + " split-video"
